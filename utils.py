...
if __name__ == "__main__":
    ...
    finding = 1
    # Read scan
    print('/data/LNDb-{:04}.mhd'.format(lnd))
    [scan,spacing,origin,transfmat] =  readMhd('scripts/data/LNDb-{:04}.mhd'.format(lnd))
    print(spacing,origin,transfmat)
    # Read segmentation mask
    [mask,spacing,origin,transfmat] =  readMhd('scripts/masks/LNDb-{:04}_rad{}.mhd'.format(lnd,rad))
    print(spacing,origin,transfmat)
    # Read nodules csv
    csvlines = readCsv('scripts/trainNodules.csv')
    header = csvlines[0]
    nodules = csvlines[1:]
    for n in nodules:
    ...
    img = scan_cube[int(scan_cube.shape[0]/2),:,:]
    imgScaled = scale(img, 0, 255)
    print(imgScaled)

  
    sift = cv2.xfeatures2d.SIFT_create() #detector
    kp, des = sift.detectAndCompute(imgScaled,None)
